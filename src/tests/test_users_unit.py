...
def test_add_user_invalid_json_key(test_app):
    client = test_app.test_client()
    resp = client.post(
        "/users",
...
def test_update_user_duplicate_email(test_app, monkeypatch):
    ...
    def mock_get_user_by_id(user_id):
        d = AttrDict()
        d.update(id=1, username="kada", email="kadaliao@gmail.com")
        return d
    ...
    resp = client.put(
        "/users/1",
        data=json.dumps(dict(username="me", email="kadaliao@gmail.com")),
        content_type="application/json",
    )
    data = json.loads(resp.data.decode())
    assert resp.status_code == 400
    assert "Sorry. That email already exist" in data["message"]
