...
def test_remove_user(test_app, test_database, add_user):
    test_database.session.query(User).delete()
    user = add_user(username="usertoremove", email="usertoremove@tdd.com")
    client = test_app.test_client()
    ...
    assert "usertoremove@tdd.com was removed!" in data["message"]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        [1, {"email": "me@tdd.com"}, 400, "Input payload validation failed"],
        [
            999,
            {"username": "me", "email": "me@tdd.com"},
            404,
            "User 999 does not exist",
        ],
    ],
)
def test_remove_user_invalid(
